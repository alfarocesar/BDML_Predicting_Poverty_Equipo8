...
# 4.3 Función para calcular nuevas variables solicitadas
calcular_variables_adicionales <- function(data) {
  # Verificar qué variables están disponibles en los datos
  vars_ingr_mes <- c("P6510", "P6545", "P6580", "P6585s1", "P6585s2", "P6585s3", 
                     "P6585s4", "P6590", "P6600", "P6610", "P6620")
  
  vars_bene_año <- c("P6630s1", "P6630s2", "P6630s3", "P6630s4", "P6630s6")
  
  vars_ingr_año <- c("P7510s1", "P7510s2", "P7510s3", "P7510s5", "P7510s6", "P7510s7")
  
  # Filtrar solo variables presentes en los datos
  vars_ingr_mes_presentes <- vars_ingr_mes[vars_ingr_mes %in% names(data)]
  vars_bene_año_presentes <- vars_bene_año[vars_bene_año %in% names(data)]
  vars_ingr_año_presentes <- vars_ingr_año[vars_ingr_año %in% names(data)]
  
  # Crear variables dependiendo de qué columnas existen en los datos
  if(length(vars_ingr_mes_presentes) > 0) {
    data <- data %>%
      mutate(
        tiene_ingr_mes = as.integer(
          rowSums(select(., all_of(vars_ingr_mes_presentes)) == 1, na.rm = TRUE) > 0
        )
      )
  } else {
    data$tiene_ingr_mes <- 0
  }
  
  if(length(vars_bene_año_presentes) > 0) {
    data <- data %>%
      mutate(
        tiene_bene_año = as.integer(
          rowSums(select(., all_of(vars_bene_año_presentes)) == 1, na.rm = TRUE) > 0
        )
      )
  } else {
    data$tiene_bene_año <- 0
  }
  
  if(length(vars_ingr_año_presentes) > 0) {
    data <- data %>%
      mutate(
        tiene_ingr_año = as.integer(
          rowSums(select(., all_of(vars_ingr_año_presentes)) == 1, na.rm = TRUE) > 0
        )
      )
  } else {
    data$tiene_ingr_año <- 0
  }
  
  return(data)
}

# 4.4 Aplicar transformaciones a los datos de personas
train_personas <- train_personas %>% 
  preprocess_binarias() %>%
  categorizar_oficio() %>%
  calcular_variables_adicionales()
...
train_hogar_agregado <- train_personas %>%
  ...
  summarize(
    ...
    max_años_educ = if (all(is.na(P6210s1))) NA_real_ else max(P6210s1, na.rm = TRUE),
    ...
    # NUEVAS VARIABLES SOLICITADAS - proporción entre personas en edad de trabajar
    otro_ingr_mes = sum(tiene_ingr_mes == 1 & Pet == 1, na.rm = TRUE) / 
      max(1, sum(Pet == 1, na.rm = TRUE)),
    otro_bene_año = sum(tiene_bene_año == 1 & Pet == 1, na.rm = TRUE) / 
      max(1, sum(Pet == 1, na.rm = TRUE)),
    otro_ingr_año = sum(tiene_ingr_año == 1 & Pet == 1, na.rm = TRUE) / 
      max(1, sum(Pet == 1, na.rm = TRUE))
  ) %>%
...
################################################################################
# 7. ANÁLISIS COMPARATIVO DE VALORES FALTANTES EN BASES UNIDAS               #
################################################################################

cat("Analizando valores faltantes en las bases unidas...\n")

# Función para calcular resumen de missing values
resumen_missing <- function(df) {
  df %>%
    summarise(across(everything(), ~sum(is.na(.)))) %>%
    pivot_longer(cols = everything(), names_to = "variable", values_to = "n_missing") %>%
    mutate(
      total_filas = nrow(df),
      pct_missing = round(100 * n_missing / total_filas, 2)
    )
}

# Calcular resúmenes de valores faltantes
train_missing <- resumen_missing(train_final)
test_missing <- resumen_missing(test_final)

# Identificar variables comunes (excluyendo 'Pobre' que solo está en train)
variables_comunes <- intersect(
  train_missing$variable[train_missing$variable != "Pobre"], 
  test_missing$variable
)

# Crear tabla comparativa completa
comparacion_missing <- train_missing %>%
  filter(variable %in% variables_comunes) %>%
  select(variable, n_missing, pct_missing) %>%
  rename(
    n_missing_train = n_missing,
    pct_missing_train = pct_missing
  ) %>%
  left_join(
    test_missing %>%
      filter(variable %in% variables_comunes) %>%
      select(variable, n_missing, pct_missing) %>%
      rename(
        n_missing_test = n_missing,
        pct_missing_test = pct_missing
      ),
    by = "variable"
  ) %>%
  mutate(
    diferencia_pct = pct_missing_test - pct_missing_train
  ) %>%
  arrange(desc(abs(diferencia_pct)))

# Mostrar la tabla completa en la consola
